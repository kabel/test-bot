...
function usage() {
    console.log(chalk`{bold Usage: ${basename(process.argv[1])}} [{underline options}] {underline build-id}

Deploy Homebrew bottle artifact from Azure DevOps Services with {underline build-id}.
...
The following configuration items must be in the environment or provided in the {bold --secrets} option.
    ...
    {bold HOMEBREW_TAP}               Homebrew tap name to deploy to
...
async function main() {
    try {
        ...
        if (Number.isNaN(buildId)) {
            console.error("build-id is not a number");
            usage();
            process.exit(1);
        }

        const expandedPath = await fetch.run({buildId: buildId, artifactName: opts.artifact, dryRun:opts.d});
        await ciUpload.run({workingPath: expandedPath, dryRun:opts.d, pr: opts.pr, noPush: opts.n});
    } catch (reason) {
        let code = reason;
